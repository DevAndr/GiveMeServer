...
import { JwtPayload, AuthResp, Tokens } from "./types";
...
const EXPIRE_TIME = 1000 * 60 * 60 * 24 * 1 //20 * 1000
...
export class AuthService {
  ...
  async signUpLocal(authDto: SignUpDto): Promise<AuthResp> {
        ...
        if (e instanceof PrismaClientKnownRequestError) {
          if (e.code === "P2002") {
            throw new ForbiddenException(
              `пользователь с email: ${authDto.email} уже существует`
            );
          }
        }
        throw e;
  ...
  async signInLocal(authDto: AuthDto): Promise<AuthResp> {
  ...
  async getTokensTwitch(code: string): Promise<TwitchResponse | null> {
    ...
    if (code) {
      const { data, status, statusText } = await axios.post<TwitchResponse>(
        `${this.config.get("URL_OAUTH_TWITCH_TOKEN")}`,
        {
          client_id: this.config.get("CLIENT_ID"),
          client_secret: this.config.get("SECRET_CLIENT"),
